...
import {
  ...
  Check, AlertCircle, Eye, EyeOff, Pencil, X, ShieldAlert,
} from "lucide-react";
...
export default function SettingsPage() {
  ...
  const handleSavePassword = async (e: React.FormEvent) => {
    ...
    if (newPass.length < 6) {
      setPassFeedback({ type: "error", msg: "A senha deve ter pelo menos 6 caracteres." });
      return;
    }
    ...
    try {
      ...
      setPassFeedback({ type: "error", msg });
    } finally {
      setSavingPass(false);
    ...
    >
      <div className="p-6 md:p-8 max-w-2xl mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Section icon={User} title="Perfil" subtitle="Seu nome e endereço de e-mail">
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-semibold text-zinc-900 dark:text-white/90">{name}</p>
                  <p className="text-sm text-zinc-500 dark:text-white/40">{email}</p>
              ...
              <AnimatePresence mode="wait">
                {!editingProfile ? (
                  <motion.button
                    ...
                    onClick={() => { setEditName(name); setEditEmail(email); setEditingProfile(true); }}
              ...
              {themeOptions.map(({ key, label, icon: Icon }) => (
                <button
                  ...
                  className={`relative flex flex-col items-center gap-2.5 p-4 rounded-xl border-2 transition-all ${
                    theme === key
                      ? "border-violet-500/60 bg-violet-500/[0.08] dark:bg-violet-500/[0.08]"
